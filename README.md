...
review/                 오탐 검수 세션 — CLI(P0) + 로컬 웹 app.py(P1) + session.py(판정)
                        export.py(오탐 영상 NAS 반영) · models.py(엣지 탐지 모델 정보)
                        월간 성능 검수 — perf_app.py + perf.py(표본·채점·집계) + perf_render.py(리포트)
                        webui.py = 두 웹 앱 공통 화면 부품 (로그인·CSS·영상·단축키·전량 업데이트)
...
scripts\review sync --dry-run -v                       # 검수 기록 → NAS 오탐 영상 반영 미리 보기 (웹 '전량 업데이트'와 같음)
scripts\review models HANIL                            # 엣지노드 탐지 모델 정보 갱신 (SSH)
```
...
- **저장** `--out` > NAS `\\192.168.33.22\DEV\2026_retrain_dataset`(접근 가능할 때) > `data/review/` 순. 경로 `{site}/{yyyy-mm-dd}/{hook|ppe}/{yyyymmdd_HHMMSS}_{bct}_{role}.mp4`(박스 영상은 `..._{role}_tg.mp4`) + `fetch.json`.
...
판정은 **정탐 / 오탐** 둘뿐이다 (2026-09-14 '애매' 제거). 오탐에는 **오탐 항목**(안전모·하네스·안전고리)을 붙인다 (2026-09-15).

- **사이드바** 넓은 화면에선 항상 펼쳐져 있다. 맨 위 사용자·로그아웃 · `☁️ 검수 자료 전량 업데이트`, 검수 화면에선 일자 이동 · 재생 속도 · 보기 · 세션 정보·영상 내보내기 · 필터.
- **일자 선택** 최근 14일 표에 날짜별 상태(⚪ 미검수 · 🟡 진행 중 · ✅ 완료) · 검수 수 · 정탐/오탐 · 항목 미분류 · BCT별 이벤트 수.
  행을 누르면 위 일자가 그 날짜로 바뀌고(일자를 바꾸면 표의 행이 선택됨), 두 번 누르면 바로 연다.
- **필터** 시간대 · 판정(승인/거부) · 사유 · BCT · Hook 점수 구간 · 클래스 미검출(점수 0) · 상태(미검수/검수됨/오탐만/항목 미분류).
  필터를 바꾸면 목록 맨 앞부터 보인다.
- **보기 모드** 사이드바에서 전환.
  - **4개씩 (2×2, 기본)** 이벤트 4개 × 카메라 2대 = 영상 8개가 한 화면에. 각 타일에 번호와 "오탐으로 표시" 체크박스.
    타일 색: 미검수 하늘색 · 미검수 출입 거부 파스텔 노랑 · 오탐 체크 연빨강 · 저장된 것은 판정색. 판정 옆에는 ❌ 인 클래스만 표시.
    키보드 `1`~`4` 타일 선택 · `P` 전부 정탐 · `N` 체크한 것 오탐(나머지 정탐) · `Z` 되돌리기(4개 묶음 단위).
    **오탐 항목**: 체크한 타일에 `오탐 내역  1 안전모 · 2 하네스 · 3 안전고리` 칩이 생긴다. 타일 번호를 누르면 그 타일이 파란 테두리로
...
- **영상** `_tg/`(박스 있는 640) 우선, 없으면 학습용. 자동·반복 재생, 기본 2배속(사이드바에서 조절). 브라우저가 못 여는 코덱이면 ffmpeg 로 H.264 변환(로컬 캐시 `data/review/_cache/`).
  한 화면의 영상(그리드 8개)은 병렬로 한꺼번에 받은 뒤("영상 불러오는 중…") 같이 띄우고, 다음 묶음 영상은 뒤에서 미리 받아 둔다.
  버튼은 모두 콜백으로 처리해 P/N/이동 한 번에 화면이 한 번만 바뀐다(분기 안 `st.rerun()` 은 예전 화면을 한 번 더 그려 두 번 넘어가 보였다).
- **판정 저장** 누를 때마다 `{저장루트}/{site}/{date}/session.json` 에 기록. 중간에 꺼도 마지막 판정 다음부터 이어서. SQLite 아님(SMB 동시쓰기 잠금 회피).
  판정한 묶음은 `history` 에 `{"ids": [...], "prev": {...}}` 하나로 들어가 되돌리기가 묶음 단위이고, 재판정이었으면 이전 판정으로 복원된다.
- **영상 내보내기** 사이드바 버튼 = 이 날짜를 NAS 에 반영. 오탐 이벤트의 학습용(박스 없음) mp4를 **오탐 항목에 맞는 카메라만** 받는다.
  안전고리 → hook 영상, 안전모·하네스 → ppe 영상, 둘 다면 둘 다, 항목 미분류면 둘 다(분류하면 정리됨).
...
키보드·표 더블클릭처럼 브라우저에서만 도는 부분은 headless Chrome 을 CDP(Node 내장 WebSocket)로 조작해 확인했다(2026-09-15).
...
- **월 선택** 월별 표에 상태(표본 없음/미검수/진행 중/완료) · 채점 수 · 제외 · 정확도 · 모델. 행 클릭으로 선택, 더블클릭으로 열기.
- **표본 만들기** 그 달 이벤트 중 판정 연동·세 클래스 점수·영상이 모두 있는 것이 후보. BCT 별로 시드 고정 셔플해 앞에서 N건(후보가 적으면 전부).
  한 달치 수집은 수십 초(9월 1~14일 9,510건 12초). 모델 버전 칸에는 현재 탐지 모델 이름이 기본으로 들어가고, 리포트·비교에 쓰인다.
- **채점 (2×2)** 타일마다 `틀린 항목  안전모 · 하네스 · 안전고리 · 제외` 칩. **모델 판정이 틀린 항목만** 누르고 `P`(저장 · 다음 4개), `Z` 되돌리기(묶음 단위).
