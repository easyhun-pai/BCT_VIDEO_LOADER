# Paimedialab 성능 검수 플랫폼 — 로컬 웹 실행기 (PowerShell)
#   .\perf_app.ps1            브라우저가 http://localhost:8502 로 열린다 (오탐 검수 :8501 과 동시에 켜도 된다)
#   Ctrl+C 로 종료.
...
$app = Join-Path $root 'review\perf_app.py'
Push-Location $root
[Environment]::CurrentDirectory = $root
try {
    & $py -m streamlit run $app --server.port 8502 @Rest
}
finally {
    Pop-Location
}
